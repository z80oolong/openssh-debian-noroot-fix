...
本差分ファイルは標準的な SSH サーバである [OpenSSH][OSSH] に適用する差分ファイルであり、 [Z.OOL. (mailto:zool@zool.jpn.org)][ZOOL] が著作権を有します。

従って、本差分ファイルは [OpenSSH][OSSH] のライセンスと同様である BSD License に基づいて配布されるものとします。詳細については、本リポジトリに同梱する ```LICENSE``` を参照して下さい。
